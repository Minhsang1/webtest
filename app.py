...
def write_system_log(user_id, action, details):
    ...
    conn.execute(
        "INSERT INTO system_logs (user_id, action, ip_address, details, created_at) VALUES (?, ?, ?, ?, ?)", 
        (user_id, action, ip, details, datetime.now())
    )
    conn.commit()
    conn.close()

# ==========================================
# CÁC ROUTE NGƯỜI DÙNG & XÁC THỰC
# ==========================================
...
def login():
    if request.method == 'POST':
        ...
        if user:
            # KIỂM TRA CHỨC NĂNG KHÓA TÀI KHOẢN TẠI ĐÂY
            if user['is_locked'] == 1:
                write_system_log(user['id'], 'LOGIN_BLOCKED', f"Tài khoản bị khóa cố đăng nhập: {user['username']}")
                return render_template('login.html', error='Tài khoản của bạn đã bị Admin khóa!')
...
def register():
    ...
    try:
        conn.execute(
            "INSERT INTO users (username, password, role, full_name, email, balance, is_locked) VALUES (?, ?, 'Customer', ?, ?, 0, 0)", 
        ...
        return redirect(url_for('login', error='Lỗi hệ thống!'))
    finally: 
        conn.close()
...
def profile():
    ...
    if request.method == 'POST':
        conn.execute(
            ...
            (request.form['full_name'], request.form['email'], request.form['password'], session['user_id'])
        )
        conn.commit()
        write_system_log(session['user_id'], 'PROFILE_UPDATE', "Cập nhật thành công")
...
def dashboard():
    ...
    return render_template('dashboard.html', username=session['username'], role=session['role'], 
                           products=products, search_query=s, cart=cart, cart_total=cart_total, 
...
def product_detail():
    ...
    cart = session.get('cart', [])
    cart_total = format_price(sum([parse_price(item['price']) for item in cart]))
    
    return render_template('product.html', username=session['username'], product=product, 
                           cart=cart, cart_total=cart_total, balance=format_price(user['balance']))
...
def checkout():
    ...
    if total_val > user['balance']:
        conn.close()
        return redirect(url_for('payment_failed', error='Số dư ví không đủ! Vui lòng nạp thêm.'))
    ...
    cur = conn.execute(
        "INSERT INTO payments (user_id, order_code, total_amount, payment_method, status, created_at) VALUES (?, ?, ?, ?, ?, ?)", 
        (session['user_id'], order_code, format_price(total_val), 'Wallet', 'Completed', datetime.now())
    )
    conn.execute(
        "INSERT INTO payment_logs (payment_id, api_endpoint, raw_request, raw_response, created_at) VALUES (?, ?, ?, ?, ?)", 
        (cur.lastrowid, PAYMENT_GATEWAY_URL, f"Pay_{total_val}", "Success", datetime.now())
    )
    ...
    session['cart'] = []
    write_system_log(session['user_id'], 'CHECKOUT_SUCCESS', order_code)
...
def payment_success(): 
    return render_template('payment_success.html', order_code=request.args.get('order_code'), total=request.args.get('total'))

@app.route('/payment_failed')
def payment_failed(): 
    return render_template('payment_failed.html', error=request.args.get('error'))
...
@app.route('/admin')
def admin_dashboard():
    if not session.get('loggedin') or session.get('role') != 'Administrator': 
...
# CHỨC NĂNG KHÓA / MỞ KHÓA TÀI KHOẢN (Đã thêm)
...
def toggle_lock(id):
    ...
    if id == session.get('user_id'): 
        return redirect(url_for('admin_dashboard', error='Bạn không thể tự khóa tài khoản Admin của chính mình!'))
    
    conn = get_db_connection()
    user = conn.execute("SELECT is_locked, username FROM users WHERE id = ?", (id,)).fetchone()
    
    if user:
        new_status = 1 if user['is_locked'] == 0 else 0
        conn.execute("UPDATE users SET is_locked = ? WHERE id = ?", (new_status, id))
        conn.commit()
        
        action_name = "Khóa" if new_status == 1 else "Mở khóa"
        write_system_log(session['user_id'], 'ADMIN_LOCK_ACCOUNT', f"{action_name} tài khoản: {user['username']}")
        
    conn.close()
    return redirect(url_for('admin_dashboard', success='Cập nhật trạng thái khóa tài khoản thành công!'))
...
def admin_add_user():
    ...
    try:
        conn.execute(
            "INSERT INTO users (username, password, role, full_name, email, balance, is_locked) VALUES (?, ?, ?, ?, ?, ?, 0)",
        ...
        return redirect(url_for('admin_dashboard', error='Tên đăng nhập đã tồn tại!'))
    finally: 
        conn.close()
...
def admin_edit_user():
    ...
    try:
        conn.execute(
            "UPDATE users SET username=?, password=?, role=?, full_name=?, email=?, balance=? WHERE id=?",
            (request.form['username'], request.form['password'], request.form['role'], 
             request.form['full_name'], request.form['email'], parse_price(request.form['balance']), request.form['user_id'])
        )
        conn.commit()
        return redirect(url_for('admin_dashboard', success='Sửa tài khoản thành công!'))
    except: 
        return redirect(url_for('admin_dashboard', error='Lỗi trong quá trình sửa tài khoản!'))
    finally: 
        conn.close()
...
def manage_balance():
    ...
    amt = parse_price(request.form.get('amount', '0'))
    ...
    conn.commit()
    conn.close()
    return redirect(url_for('admin_dashboard', success='Điều chỉnh số dư thành công!'))
...
def add_product():
    ...
    conn = get_db_connection()
    conn.execute(
        "INSERT INTO products (name, price, image_url, rating, sold) VALUES (?, ?, ?, '5.0', '0')", 
